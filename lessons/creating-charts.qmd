...
Then we pass the inaturalist observation to `ggplot()` function using the `data` argument.

```{r, pass_data_to_ggplot}
ggplot(data = inat_year)
```

We need to tell **ggplot** how to use data to create a plot. We tell **ggplot** how to map the data to various plot elements, such as x/y axis, size, or color by using the `aes()` function. `aes` is short for aesthetic mappings.

For bar charts, we need to tell what column to use for the x axis.

We want to create a plot with `year` on the x axis so we use `mapping = aes(x = year)`. **ggplot** will count the number of rows for each year, and use the number of rows for y axis.
...
In the previous example we created a data frame `inat_year_count` that has observations per year. To create bar chart when we already have values for x and y, use `geom_col()`
...
my_chart <- ggplot(data = my_obs_by_year, mapping = aes(x = year)) +
  geom_bar()
