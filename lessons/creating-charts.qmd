...
**`ggplot2`** is a powerful package that allows you to create complex charts from tabular data (data in a table format with rows and columns). The **`gg`** in **`ggplot2`** stands for “grammar of graphics”, and the package uses consistent vocabulary to create plots step by step by adding new layers, which allows for extensive flexibility and customization of plots.
...
We need to add a year column to iNaturalist data.
...
Then we pass the data frame `inat_year` to `ggplot()` function as the first argument.
...
For bar charts, we need to tell what field to use for the x axis. We want to create a plot with `year` on the x axis so we use set the `aes()` function `x` argument to `year`. **ggplot** will count the number of rows for each year, and use the number of rows for the y axis.

```{r, pass_aes_to_ggplot}
ggplot(inat_year, aes(x = year))
```
...
For `aes`, use `year` on the `x` axis and `count` on the `y` axis. And we use `geom_line()` to create a line chart.

```{r, create_line_chart}
ggplot(inat_year_count, 
       aes(x = year, y=count)) +
  geom_line()
```
...
ggplot(my_obs_by_year, aes(x = year)) +
  geom_bar()
```
:::
::::
...
We can create stacked bar charts by adding a field from the dataframe to `aes()` function `fill` argument. Let's create a stacked bar chart using `quality_grade`.

```{r, create_stacked_bar_chart}
ggplot(inat_year, aes(x = year, fill = quality_grade)) +
  geom_bar()   
```
...
ggplot(year_quality_count, 
         aes(x = year, y = count, color = quality_grade)) +
  geom_line()
```
...
ggplot(inat_year, aes(x = year, fill = year)) +
  geom_bar()  
```
...
Let's convert year into a factor.

```{r create_chart_with_fill_color_year}
ggplot(inat_year, aes(x = year, fill= factor(year))) +
  geom_bar()  
```
...
myplot <- ggplot(inat_year,  aes(x = year)) +
  geom_bar(fill='#75cd5e') +
  scale_x_continuous(n.breaks = 8) 
...
my_chart <- ggplot(my_obs_by_year, aes(x = year)) +
  geom_bar()
