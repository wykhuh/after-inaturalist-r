...
First step is load the R packages. These packages provide functions that we need to analyze the City Nature Challenge data.
...
First I need to import the City Nature Challenge iNaturalist CSV. `read_csv()` reads the csv. `here()` helps deal with file paths. I use assignment arrow `<-` to asssign the content from `read_csv()`  to an object called `inat_data`.
...
I use `names()` to see the column names in the dataframe.

```{r}
names(inat_data)
```

I use `filter()` to select which rows I want. I pass in the column names and column values to `filter()`.

I want observations for 'Great Blue Heron'. I search iNaturalist for 'great blue heron', and go to its [taxa page](https://www.inaturalist.org/taxa/4956-Ardea-herodias) since I want to use the Latin scientific names in my `filter()` query. The kingdom is 'Animalia' and the species is 'Ardea herodias'.
...
I add `filter()` for research grade and unobscured locations.
...
I save the iNaturalist observations as a CSV to the results folder using `write_csv()` and `here()`. By default, `NA` values will be saved as 'NA'. I don't want 'NA' in the CSV, so I set `na=''`.
...
Since I want to create a map, I need to use `st_as_sf()` to convert the dataframe into a sf object with a `geometry` column. I pass in "longitude" and "latitude" to `coords` so that `st_as_sf()` know which columns are the longitude and latitude columns. I set the CRS as 4326.
...
I can double click `water_areas` in the Enviroment to look at content. Each body of water has a separate row. Some records have names in `FULLNAME`, some do not. 
...
Since the CRS are different, I need to use `st_transform()` to change CRS for the water areas so it is the same as heron observations. I pass in the `water_areas` sf object and use `st_crs()` to get the CRS for `heron_obs_sf`. 
...
Now that the CRS are the same, I can add the water areas and heron observations to the map. I set the color for the iNaturalist observations to green to make them easier to see.
...
I want to get observations within 1/2 mile of a body of water. This means I need to create a buffer 1/2 mile for all bodies of water and find observations inside the buffers.
...
Then I can create a map with the observations and buffers.
...
I can save the map as an image. First I assign the assign code for map to an object called `heron_near_water_map`.

```{r}
#| eval: false
heron_near_water_map <- mapview(heron_near_water_sf, col.regions='green') +
  mapview(buffer_water_areas)
```

Then I use `mapshot2()` to save `heron_near_water_map` in the results folder as .png image. 

```{r}
#| eval: false
mapshot2(heron_near_water_map, file = here('results/heron_near_water_draft.png'))
```
...
#| eval: false
mapview(water_areas) +
  mapview(heron_obs_sf, col.region = 'green')
```
...
Next I want to update the appearance of the chart. I use `labs()` to add titles, subtitle, and axis labels. I use `fill='#77b100'` to set the fill color of the bars to iNaturlist green color. I use `theme_bw()` and `theme()` to change overall appearance of the chart.
...
I want to create a nice looking map for my presentation. ggplot offers more ways to customize the look of the map than mapview, so I use ggplot to create a map.
...
I use `ggsave()` to save maps created with ggplot. I pass in the file path and `heron_final_map`. I save the image in the results folder. The file path ends with .jpg because I want a jpg image. I also set the height and width for the image.
