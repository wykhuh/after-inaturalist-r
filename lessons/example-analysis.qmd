...
Goal: I want all research grade observations for Great Blue Heron. I want to observations near bodies of water and observations not near bodies of water.
...
I want to look at heron obsevations near water. First I need to load the file with the water areas.
...
I can use `lengths(st_intersects()) >0` to get the observations within the water buffer. That will produce a vector with TRUE/FALSE values. I can use `mutate()` to add a `near_water` column that stores the TRUE/FALSE values.
...
And I save the map as an image. I use `mapshot2()` to save mapview interactive maps. We can save the image as .png or .jpg
...
I to create a nice looking map for my presentation. I want to add more information and titles. ggplot offers more ways to customize the look of the map than mapview, so I use ggplot to create a map.
...
I create a map with LA County boundaries, water areas, and heron observations. I color code the observations based on if they are near water.
