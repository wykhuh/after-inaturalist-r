...
I use `ggsave()` to save charts created with ggplot. I pass in the file path and `heron_chart`. I save the image in the results folder. The file path ends with .jpg because I want a jpg image. I also set the height and width for the image.
...
I use `filter()` to select observations that have 'CC0', 'CC-BY', or 'CC-BY-NC' license. I use `!is.na(image_url)` to only get records where `image_url` is not NA, meaning the observation has an image. I use `slice_sample()` to get 3 random observations. Then I use `download_inaturalist_images()` to download images. The images are saved in 'results/images'.
