...
We can use `dim()` dimension function from **R** to get the dimension of a data frame.
...
#### Find species with fuzzy search

Previously we used `common_name == 'Western Fence Lizard'` which did an exact match for `'Western Fence Lizard'.` If you want to do a search that contains a phrase, use str_subset`()` from **stringr** package.

Let's find all species common names with the word 'lizard'.
...
When we use `pattern = 'lizard'`, we get zero results. The reason is that `str_subset()` is case sensitive. It is looking for matches for lowercase 'lizard'.
...
As you can see, all the common names with 'lizard' use uppercase 'Lizard'. This is why the first search returned zero results. There are no common names with lowercase 'lizard'
...
The results return names with the word 'plants' because it 'ants' is part of 'pl**ants**'.

If we want to only find matches for the word 'ants' and ignore partial matches, we need to add `\\b` at the before and after 'ants'.
