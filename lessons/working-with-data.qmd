...
Our `inat_data` data frame has 37 columns. We want three columns: `user_login`, `common_name`, `scientific_name`, `observed_on`.
...
In addition to coordinates being intentionally obscured, another thing that can affect the coordinates for an observation is the accuracy of the coordinates. The accuracy of GPS on smart phones depends on the hardware, software, physical environment, etc. The `positional_accuracy` from iNaturalist measures the coordinates error in meters.
