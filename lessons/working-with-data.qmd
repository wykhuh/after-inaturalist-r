...
iNaturalist FAQ about [quality grade](https://help.inaturalist.org/en/support/solutions/articles/151000169936-what-is-the-data-quality-assessment-and-how-do-observations-qualify-to-become-research-grade-).
...
Let's see the unique common names. We get oaks species and oaks genus.
...
I don't have enough expertise to know if a taxa name is unique, so to play it safe, I list the entire taxonomy hierarchy.
...
Instead of searching the entire hierarchy, someone with more expertise might know there is only one Quercus in Plantae so they search for Plantae kingdom and genus Quercus.
...
We can use `mutate()` from **dplyr** and `year()` from **lubridate** to add a `year` column.
