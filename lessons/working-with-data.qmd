...
Let's look for observations for all oak trees.
...
When you search for 'oaks' on iNaturalist, click the 'About' link, and it will bring you to the [taxa page for 'oaks'](https://www.inaturalist.org/taxa/47851-Quercus). 'Oaks' is the common name for genus Quercus. When you search iNaturalist observations for 'oaks', iNaturalist returns observations that belong to the genus Quercus such as 'Live Coast Oak' and 'Valley Oak'. To match the iNaturalist search results, we need to look for all observations where the genus equals Quercus.
...
::: callout-note
The reason why iNaturalist has two more observations for 'oaks' is because there are two observations which don't have latitude and longitude data. Our cleaned iNaturalist data excludes observations without latitude and longitude.
:::

Whenever we need to search for higher taxonomy groups, we need to use the iNaturalist site to get the taxon rank and Latin name. 

Another example, let's search for 'trees'. iNaturalist taxa page for 'trees' is [Phylum Tracheophyta](https://www.inaturalist.org/taxa/211194-Tracheophyta). We need to do search using `taxon_phylum_name` and 'Tracheophyta'
...
dim(trees_obs)
```
There are 95372 tree observations.
...
Let's get observations where `common_name` is 'Western Fence Lizard' or `common_name` is 'Western Honey Bee'.
...
** & (and)** return rows where all conditions are true
...
-   observations where user_login is 'natureinla' and common_name is 'Western Fence Lizard'
-   79 rows with 1 user_login and 1 common_name


**| (or)** returns rows where any conditions are true.
...
```
-   observations where user_login is 'natureinla' plus observations where common_name is 'Western Fence Lizard'
-   6,216 rows with 1052 user_login and 1031 common_name
...
Let's get use `year()` to get the year from `observed_on` for rows 10300 to 10320
...
Let's get all observations for 2020.
