...
`read_csv()` will return the content of the file. In order for us access the content later on, we will assign the content to an object called `inat_data`. `inat_data` is displayed in the **Environment** panel. Double click on `inat_data` in **Environment** to see all the data.
...
Use `glimpse()` from **dplyr** to see a information about a data frame. It shows the number of rows and columns. For each column, we see the name, data type (**dbl** for number, **chr** for character, **lgl** for logical (TRUE, FALSE), **date** for date), and the first few values.

```{r, call_glimpse}
glimpse(inat_data)
```

You can use `View()` from **R** to view the data frame in a new viewer.
...
To use the `filter()` function, the first agrument is the name of the data frame, and the second argument is the selection criteria.
...
Pipe operator take the thing on the lefthand side and insert it as the first argument of the function on the righthand side.`inat_data` gets piped into a `filter()` function, where it is modified. The output from `filter()` is then gets sent into the `select()` function, where it is further modified. It can also be helpful to think of `%>%` as meaning "and then".
...
`select()` creates a data frame with four fields. 

```{r show_4_columns_from_select}
inat_data %>%
  select(user_login, common_name, scientific_name, observed_on) %>%
  names
```
When we try to `filter()` using `coordinates_obscured`, we get an error because the 4-field data frame we pass to `filter()` does not have the field `coordinates_obscured`.
:::
...
```{r, save_file}
#| eval: false
write_csv(top_ten, 'results/top_ten_observations.csv', na='')
```
