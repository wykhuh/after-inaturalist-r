...
The **Arguments** section for `head()` shows four arguments. The first argument `x` is required, the rest are optional. For example, the `n` argument in `head()` specifies the number of rows to print. It defaults to 6, but we can override that by specifying a different number:

```{r}
head(x = inat_data, n = 10)
```

If we order the argument the same order they are listed in **Arguments** section, we don’t have to name them:
...
Our `inat_data` dataframe has 37 columns. We want four columns: `user_login`, `common_name`, `scientific_name`, `observed_on`.
...
We are writing instructions for the computer. If there is a typo, mispelling, pass in wrong arguments into functions, etc, the code will not work and we will see errors. R will display the errors in red. You need to fix the errors in order for the code to work. Here are some example errors.
...
Let's get all 'Western Fence Lizard' observations that are research grade.
...
#### select observations by common_name and user_login

Let's get 'Western Fence Lizard' observations made by 'natureinla'.
...
In addition to coordinates being intentionally obscured, another thing that can affect the coordinates for an observation is the accuracy of the coordinates. The `positional_accuracy` from iNaturalist measures the coordinates error in meters. The accuracy of GPS on smart phones can depend on the hardware, software, physcial environment, etc.
...
inat_data %>%
  ...
  select(user_login, common_name, scientific_name, positional_accuracy)
...
A vector is how R stores a list of items. `c()` is a function from R that creates a vector with the passed in arguments. `%in%` operator from R checks if a vector is contained in another vector.

#### select observations by multiple license 

iNaturalist observations, photos, and sounds are covered by licenses. The license determines how your data is used. The default license is **CC BY-NC** (Creative Commons: Attribution-NonCommercial) so other people can use the content if they give attribution to you and use it for non-commercial purposes. iNaturalist only exports observations  with  No Copyright (CC0), Attribution (CC BY), and Attribution-NonCommercial (CC BY-NC) license to [Global Biodiversity Information Facility (GBIF)](https://www.gbif.org), an international organization that provides access to biodiversity information. Many researchers who use iNaturalist data get their data from GBIF because GBIF provides [DOI](https://www.doi.org) that researchers can cite in their papers.  More info about [iNaturalist licenses](https://help.inaturalist.org/en/support/solutions/articles/151000175695-what-are-licenses-how-can-i-update-the-licenses-on-my-content-).
...
```
 ...
 Let's get observations with CC0, CC-BY, or CC BY-NC license. This codes will find observations where `license` is in the vector with these values: 'CC0', 'CC-BY', 'CC BY-NC'
...
Sometimes we want to use both `&` `|` to select the rows. You can use multiple filter statememts.
...
-   Use select() to pick 4 columns ::: answer
