...
**sf** handles geospatial files with vector data. **sf** package uses a special type of data frame called `sf` object (special features) to store vector data.

**mapview** creates interactive maps.
...
We have 'data.frame' and 'tbl' like with `read_csv()`. We also have an additional `sf` object. The **sf** package uses a special type of data frame called `sf` (special features) to store vector data. Because `sf` is based on data frames, we can use the same types of commands on `sf` objects that we used for tibbles and data frames.
...
The following code returns the number of items in each row of `inat_sf` that is within `expo_park_boundary`. Since each row only contains one location, we will either get 1 or empty.

```{r, st_intersects}
st_intersects(inat_sf, expo_park_boundary)
```

We use `lengths(st_intersects(inat_sf, expo_park_boundary)) > 0` to check it the row has items within Exposition Park. If a row has 1 item inside `expo_park_boundary`, return TRUE. Otherwise return FALSE. This will create a vector of with TRUE and FALSE values, also known as logical vector.
...
This code selects all rows in `inat_sf` where there is one or more items in the row that are inside `expo_park_boundary`
