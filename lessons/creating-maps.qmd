...
-   `crs` argument sets the [coordinate reference system](intro-science-coding.qmd#coordinate-reference-systems) (CRS). 4326 is the code for the EPSG:4326 CRS, a commonly used CRS.
-   `st_as_sf()`removes the `longitude` and `latitude` columns and add a `geometry` column.
...
**ggplot2** is a package to create visualizations for data. **ggplot2** is most commonly used for charts, but it can also be used to create static maps. **ggplot2** creates charts and map by adding new layers.

Let's create a map for Coast Live Oak observations.

Call `ggplot()` to start a map. Then we use `+` to add a new layer to the map. We pass the data to `geom_sf()` using the `data` argument. `geom_sf()` uses the information in the `geometry` column to plot each row.
...
There are various places where you can download geospatial files for free. I downloaded the LA County boundaries from [LA City Geohub](https://geohub.lacity.org/datasets/lacounty::la-county-boundary-7/about). Geohub offers files in various formats including CSV, Shapefile, GeoJSON, and KML. We are using the Shapefile.

`st_read()` function from **sf** package can read files and databases in various formats. We will use `st_read()` to read the LA County boundary file.

```{r, get_LA_County_boundaries}
la_county_boundary <- st_read('data/raw/LA_County_Boundary/LA_County_Boundary.shp')
```
...
Shapefiles include a CRS and `geometry` column.
...
Since the CRS are different, we need to use `st_transform()` to change the CRS of the LA County boundary.
...
Let's create a static map with LA County and oak observations. Create a new layer for each dataset using `+`.
...
We can add a title to the map using `ggtitle()`, and change the legend title using `labs()`. We can add `theme_void()` to get rid of the grey background and axis labels.

```{r add_title_to_static_map}
ggplot() +
  geom_sf(data = la_county_boundary, color="black", fill='beige')  +
  geom_sf(data = inat_oak_map, mapping=aes(color=quality_grade)) +
  ggtitle('CNC observations for Live Coast Oaks in LA County',
          subtitle='2016-2024') +
...
## Map observations for a specific area
...
Let's use `st_read`() to read a geojson file.
...
The Expo Park boundary file has a CRS and `geometry` column.
...
la_neighborhoods <- st_read('data/raw/la_times_la_county_neighborhoods.json')
```
...
Use `select()` to pick the name amd geometry columns.
...
Ther is a bug with **sf** <https://github.com/r-spatial/sf/issues/1762>. This bit a code is fix for the bug.
...
181K out of the 191K CNC observations are intersect one of the neigborhood boundaries.
...
`aes(fill=xxx)` sets which column to use determine the color of each neighborhood. `labs(fill='xxx')` sets the title show in the legend.
...
`zcol` sets which column to use determine the color of each neighborhood. `layer.name` sets the title show in the legend.
