...
3.  Use `filter()` and `select()` to get the rows and columns we want.
4.  Create map.
...
Let's create a mapp for all the observations.
...
Let's add LA County to the maps.
...
`st_intersects()` from **sf** can tell us which points are inside a boundary. 
`st_intersects()` returns number of items  in `inat_map` that is inside `expo_park` for every row.
