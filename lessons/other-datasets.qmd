...
Let's compare iNaturalist data with the location of national, state, county, and city parks.
...
```{r, get_parks_data}
parks_national <- st_read('data/cleaned/nps_la_county.geojson')

parks_state <- st_read('data/cleaned/state_parks_los_angeles_county/state_parks_los_angeles_county.shp')

parks_county <- st_read('data/raw/DPR_Park_Facilities_View_(Accessible_Parks)/DPR_Park_Facilities_View_(Accessible_Parks).shp')

parks_city <- st_read('data/raw/Los_Angeles_Recreation_and_Parks_Boundaries/Los_Angeles_Recreation_and_Parks_Boundaries.shp')
```

We want to convert the CRS of the parks data to match the CRS of the iNaturalist data.

```{r, update_national_crs}
...
```{r}
nc_boundaries <- st_read('data/raw/Neighborhood_Councils_(Certified)/Neighborhood_Councils_(Certified).shp')
```
...
```{r}
admin_boundaries <- st_read('data/raw/admin_dist_SDE_DIST_DRP_CITY_COMM_BDY_-2349953032962506288/admin_dist_SDE_DIST_DRP_CITY_COMM_BDY.shp')
```
...
```{r}
ejsm <- st_read('data/raw/EJSM_Scores-shp/6cbc6914-690f-48ec-a54f-2649a8ddb321202041-1-139ir98.m1ys.shp')
```
