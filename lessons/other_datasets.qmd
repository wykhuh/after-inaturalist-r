...
imagesAdd a geometry column using `st_as_sf().` Select observations for 'Quercus agrifolia' aka Coast Live Oak using `filter()`.
...
2.  When you click on region, the popup shows too many fields. To select the columns displayed in popups, we can set the columns using `dataframe[c('col_1', 'col_2')]`. In R, a vector is a list of items. `c()` is a function from R that creates a vector from a list of values.
...
Create map with parks and iNaturalist data.
