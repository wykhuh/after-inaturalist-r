...
inat_raw <- read.csv(observations_raw_path)
...
We want to save the cleaned up data set so we can use it later. We can save data.frame to a CSV using the `write.csv()` function from the **R**. The first argument is the name of the data.frame, and the second is the path to the new file we want to create.

```{r}
#| eval: false
write.csv(inat, file= observations_cleaned_path, row.names=FALSE)
```
...
inat_data <- read.csv(observations_cleaned_path)
threatened_raw <- read.csv(threatened_raw_path)
introduced_raw <- read.csv(introduced_raw_path)
native_raw <- read.csv(native_raw_path)
...
```{r}
#| eval: false
write.csv(combined_data, observations_cleaned_path, row.names=FALSE)
```
